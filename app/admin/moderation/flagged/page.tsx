...
export default async function FlaggedChatsPage() {
  const service = createServiceClient()

  // Pull all flags, then aggregate in-memory.
  const { data: flags } = await service
    .from('flagged_messages')
    .select('id, message_id, conversation_id, matched_pattern, created_at')
  ...
  return (
    <>
      ...
      <ModerationTabs flaggedCount={flaggedConversations.length} />
      ...
      {flaggedConversations.length === 0 ? (
        <div className="text-center py-16 text-text-muted bg-bg-card border border-border rounded-2xl">
          ...
          <p className="font-medium text-text-secondary">No flagged conversations</p>
