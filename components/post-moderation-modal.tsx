  ...
  return (
    ...
    >
      <span
        className={[
          "flex-1 truncate",
          flagged ? "font-semibold text-error" : "text-text-secondary",
        ].join(" ")}
      >
        {label}
      </span>
      <div className="flex-1 h-1.5 rounded-full bg-bg-elevated overflow-hidden">
        <div
          className={["h-full", scoreColor(score)].join(" ")}
      ...
      <span
        className={[
          ...
          flagged ? "text-error font-semibold" : "text-text-muted",
        ].join(" ")}
      >
        {pct(score)}
      </span>
    ...
    <div className="flex flex-col gap-3 p-4 rounded-xl border border-border bg-bg-base">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {item.flagged ? (
              <span className="text-xs px-1.5 py-0.5 rounded bg-error/20 text-error font-semibold">
                flagged
              </span>
            ) : item.scanned ? (
          ...
          {item.categories.length > 0 && (
            <p className="text-xs text-error">{item.categories.join(", ")}</p>
          )}
        </div>
      </div>
      ...
      {item.type === "video" && (
        <p className="text-xs text-text-muted italic">
          Videos are not currently scanned.
        </p>
      )}
                  ...
                  <div className="flex flex-col gap-0.5">
                    {realFaces.map((f, i) => (
                      <ScoreRow
                        ...
                        flagged={
                          minorFlagged && (f.attributes?.age?.minor ?? 0) >= 0.5
                        }
                      />
                    ))}
                    {fakeFaces.map((f, i) => (
...
export function PostModerationModal({
  postId,
  onClose,
}: PostModerationModalProps) {
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            ...
            <p className="text-xs text-text-muted">
              Per-image scores. Highlighted rows exceeded the auto-flag
              threshold.
            </p>
          </div>
          <button
